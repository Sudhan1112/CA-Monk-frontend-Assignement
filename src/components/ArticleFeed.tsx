...
export default function ArticleFeed({ searchQuery = '', category = 'Latest News' }: ArticleFeedProps) {
  const [articles, setArticles] = useState<BloggerPost[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filteredArticles, setFilteredArticles] = useState<BloggerPost[]>([]);
    ...
    const fetchArticles = async () => {
      try {
        ...
        const response = await fetch(
          `https://www.googleapis.com/blogger/v3/blogs/${blogId}/posts?key=${apiKey}&maxResults=10&orderBy=published`
        );
        ...
        if (data.items) {
          ...
          const parsedArticles = data.items.map((post: any) => {
