...
export default function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    <article className="bg-card rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-border">
      {/* Article Image */}
      {article.imageUrl && (
      ...
      <div className="p-4">
        ...
        <button 
          ...
          className="text-xl font-bold text-card-foreground mb-2 line-clamp-2 hover:text-primary cursor-pointer transition-colors text-left w-full"
        >
          {article.title}
        </button>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          <div className="flex items-center gap-2">
            {article.author.image?.url && (
              <img
                src={article.author.image.url || "/placeholder.svg"}
